...
if (!empty($_POST)) // $_POST est une superglobale donc elle existe toujours !
{
	...
	if($nom_pays != NULL && $num_dev != NULL) // vérifie les champs non remplis
	{
    $nom_pays = $bdd->real_escape_string($nom_pays);
	$num_dev = intval($num_dev);

		// On vérifie si c'est une modification ou pas
		if(empty($id) || $id == 0)
		{
			// Vérification si n'éxiste pas déjà
			if(existe_pays($bdd, $nom_pays) === true)
			 {
		...
		else
		{
			...
			update_pays($bdd, $id, $nom_pays, $num_dev);
			$Session->detroyForm(); // detruit $_SESSION['sauvegarde'] si existe
